...
public class DoubleLockPattern {
    ...
    public static DoubleLockPattern getInstance() {
        if (_instance == null) {
            _instance = new DoubleLockPattern();
        }
        return _instance;
    }

    /*
     * 版本二:加锁其实只是第一次调用需要,之后调用不需要加，这种方式每次调用都加锁影响程序执行的效率
     */
    public static synchronized DoubleLockPattern getInstanceTS() {
        if (_instance == null) {
            _instance = new DoubleLockPattern();
        }
        return _instance;
    ...
    /*
        ...
        3、初始化对象
     */
    public static DoubleLockPattern getInstanceDC() {
    ...
    public static void main(String[] args) {
        DoubleLockPattern d1 = DoubleLockPattern.getInstanceTS();
        DoubleLockPattern d2 = DoubleLockPattern.getInstanceTS();
